...
static int eeprom_read_mode=0;
static int eeprom_write_mode[5]={0};
		       ...
		       const char *buf, size_t count)
{
	...
	//printk("%s:(%d)\n",attr->attr.name,calibrationSetting);	
	return count;
}
...
static void ar1021_spi_eepromwritedata(struct work_struct *work,int eeprom_offset)
{
	struct ar1021_spi_priv *priv =
		container_of(work, struct ar1021_spi_priv, work);
	char buff[16];
	...
	if(eeprom_offset<4)
	{
		...
		//printk("ar1021_spi_eepromwritedata(%d)",eeprom_offset);
		for (i=0;i<8;i++)
		{
			//printk("0x%02x ",buff[i]);
			spi_write(priv->client,(unsigned char *)&buff[i],1);
...
static void ar1021_spi_readdata(struct work_struct *work)
{
	...
	if (commandMode)
	{
		...
		//printk(KERN_DEBUG "ar1021 SPI: command response: %s",receiveBuffer);
	...
	if(eeprom_write_mode[4])
	{
		for(i=0;i<5;i++)
		{
			if(eeprom_write_mode[i])
			{
				...
				ar1021_spi_eepromwritedata(work,i);
				break;
			}
		}
		printk("ar1021_spi_eepromwritedata(%d):%d\n",eeprom_write_mode[i],i);
		return;
	}
#else
	for(i=0;i<5;i++)
	...
	if(eeprom_read_mode)
	{
		...
		//printk("ar1021 SPI: command response:  0x%02x",buff[0]);
		//printk(" 0x%02x",buff[1]);
		for (i=0;i<buff[1];i++)
		{
			spi_read(priv->client,&buff[i+2],1);
			//printk(" 0x%02x",buff[i+2]);
		}
		//printk(" \n");
		...
		switch(eeprom_read_mode)
		{
			case 1:
				eeprom_minX=ret;
				printk("ar1021 SPI: eeprom_minX(%d): %d",eeprom_read_mode,eeprom_minX);
				break;
			case 2:
				eeprom_minY=ret;
				printk("ar1021 SPI: eeprom_minY(%d): %d",eeprom_read_mode,eeprom_minY);
				break;
			case 3:
				eeprom_maxX=ret;
				printk("ar1021 SPI: eeprom_maxX(%d): %d",eeprom_read_mode,eeprom_maxX);
				break;
			case 4:
				eeprom_maxY=ret;
				printk("ar1021 SPI: eeprom_maxY(%d): %d",eeprom_read_mode,eeprom_maxY);
				break;
			case 5:
				if(buff[4]==buff[5] && buff[4]<=0x07)
				{
					...
					printk("ar1021 SPI: eeprom_read_mode(%d) error",eeprom_read_mode);
				eeprom_read_mode=0;
				return;
				break;
				
		}
		ar1021_spi_eepromdata(work,eeprom_read_mode);
		eeprom_read_mode++;
		return;
	}
...
//	printk("ar1021 SPI: ar1021_spi_probe: begin\n");
//	if (client->max_speed_hz > 50000 || client->max_speed_hz<10000) {//MAX_SPI_FREQ_HZ
//		client->max_speed_hz =30000;
//	}
...
//	printk(KERN_DEBUG "boot_command_line:%s\r\n",boot_command_line);
	msleep(2);
	buff[0]=0x55;
	buff[1]=0x06;
	buff[2]=0x21;
	...
	//printk("\r\n ");
//#endif
//	request_irq(priv->irq, touch_irq_handler_func, IRQF_TRIGGER_RISING, "ar1021_spi", priv);
	msleep(2);
