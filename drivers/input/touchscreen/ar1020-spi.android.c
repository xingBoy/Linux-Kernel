	...
	****************************************************/		
	...
	switch ((*index)++) {
		...
		case (MCHIP_MAX_LENGTH - 1):
			...
			//----------add for virtual keys
			f_x = calX;
			f_y = calY;
			if (f_x > ((4095/100) *98))
			{
#if 0
				if ((f_y >= (4095 *( 25-3)/100)) && (f_y <=(4095 *( 25+3)/100)))
				{
					if ((menukeydown == 0) && (button == 1))
					{
						input_event(priv->input, EV_KEY,KEY_MENU, 1);
						menukeydown = 1;
					}
				}
#endif 
				if ((f_y >= (4095 *( 50-3)/100)) && (f_y <=(4095 *( 50+3)/100)))
				{
					if ((homekeydown == 0) && (button == 1))
					{
						input_event(priv->input, EV_KEY,KEY_HOME, 1);
						homekeydown= 1;
					}
				}
				if ((f_y >= (4095 *( 75-3)/100)) && (f_y <=(4095 *( 75+3)/100)))
				{ 
					if ((backkeydown == 0) && (button == 1))
					{
						input_event(priv->input, EV_KEY,KEY_BACK, 1);
						backkeydown= 1;
					}
				}
			}
			
			if((backkeydown==1) && (button == 0))  
			{  
			    backkeydown=0;  
			    input_event(priv->input, EV_KEY, KEY_BACK, 0);  
			    printk("back key up\n");  
			}  
			if((homekeydown==1) && (button == 0))  
			{  
			    homekeydown=0;  
			    input_event(priv->input, EV_KEY, KEY_HOME, 0);  
			   printk("home key up\n");  
	        }  
#if 0
			if((menukeydown==1) && (button == 0))  
	        {  
	            menukeydown=0;  
	            input_event(priv->input, EV_KEY, KEY_MENU, 0);  
	            printk("menu key up\n");  
	        }  
#endif
			//----------add for virtual keys ---end
