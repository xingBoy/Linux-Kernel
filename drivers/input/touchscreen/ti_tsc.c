...
int pen = 1;
int pen_up_event_flag = 1; /*add by danny zhao*/
...
static void tsc_step_config(struct tscadc *ts_dev)
{
	...
	total_steps = 2 * ts_dev->steps_to_config;   //10
...
//						printk("<debug>z = %d, val_x = %d, val_y = %d \n",z, val_x,val_y);
					}
				}
				pen_up_event_flag=0;
			}
...
//					bckup_y = 0;
					...
					input_sync(input_dev);
					save_x=save_y=0;
					//printk("<danny debug>report btn_touch = 0\n");
				}
			}
...
//					bckup_y = 0;
			...
			/*add by danny zhao end*/


			if (pen == 0) 
			{
				if ((diffx < 15) && (diffy < 15) && (z <= MAX_12BIT)) 
				{
					input_report_abs(input_dev, ABS_X, val_x);
					input_report_abs(input_dev, ABS_Y,val_y);
					input_report_abs(input_dev, ABS_PRESSURE,5);
					input_report_key(input_dev, BTN_TOUCH,1);
					input_sync(input_dev);
//					printk("<danny debug>z = %d, val_x = %d, val_y = %d \n",z, val_x,val_y);
				}
			}
#endif
		}
