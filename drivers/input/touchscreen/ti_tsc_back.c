...
//			printk("<danny debug> cur_x=%d,cur_y=%d \n",cur_diff_x ,cur_diff_y );
		if (val_x > bckup_x) {
			diffx = val_x - bckup_x;
			diffy = val_y - bckup_y;
		} else {
		...
		if ((z1 != 0) && (z2 != 0)) {
			...
			if (pen == 0) {
				if ((diffx < 15) && (diffy < 15)
					...
					printk("<danny debug> X=%d,Y=%d BIT_TOUCH = 0\n",val_x,val_y);
				}
			}
		}
	...
	if (status & TSCADC_IRQENB_PENUP) {
		/* Pen up event */
		printk("<danny debug> tsadc_irqenb_penup  \n");
		...
		if (fsm == 0x10) {
			...
			input_sync(input_dev);
			printk("<danny debug> input BIT_TOUCH = 0\n");
		} else {
			pen = 0;
		}
