...
//    printk("zengjf cpsw_interrupte position.\n");
...
/*   
//  Modify static void _cpsw_adjust_link()
...
//		 enable forwarding 
		...
		if (phy->speed == 100)
			mac_control |= BIT(15);
      //    	printk(KERN_ALERT "*link (100)\n",%d);
        if (phy->duplex)
			mac_control |= BIT(0);	// FULLDUPLEXEN	

        if (phy->speed == 10)
			mac_control |= BIT(18); // In Band mode 
	 //       printk(KERN_ALERT "*link (10)\n",%d);
        if (phy->speed == 1000) {
			mac_control |= BIT(7);	// Enable gigabit mode 
		}
     //       printk(KERN_ALERT "*link (1000)\n",%d);
...
static int __devinit cpsw_probe(struct platform_device *pdev)
{
    ...
    printk("zengjf check %s priv->bus_freq_mhz: %d.\n", __func__, priv->bus_freq_mhz);
