...
{
    u32  out_value[2] = {100};
	int irq_gpio, irq_flags, irq, wk_major, wk_minor;
    const char *wk_driver_name, *wk_dev_name;

#ifdef _DEBUG_WK2XXX
    printk(KERN_ERR "-rockchip_spi_parse_dt()------in---\n");
#endif

    of_property_read_string(dev->of_node, "driver_name", &wk_driver_name);
    of_property_read_string(dev->of_node, "dev_name", &wk_dev_name);
    of_property_read_u32(dev->of_node, "major", &wk_major);
    of_property_read_u32(dev->of_node, "minor", &wk_minor);

    strcpy(wk2xxx_uart_driver.driver_name, wk_driver_name);
    strcpy(wk2xxx_uart_driver.dev_name, wk_dev_name);
    wk2xxx_uart_driver.major = wk_major;
    wk2xxx_uart_driver.minor = wk_minor;

	//从设备树获取IRQ——GPIO
	//irq_gpio = of_get_named_gpio_flags(dev->of_node, "irq_gpio", 0,(enum of_gpio_flags *)&irq_flags);
    of_property_read_u32_array(dev->of_node, "irq_gpio", out_value, 2);
    irq_gpio = out_value[0];
