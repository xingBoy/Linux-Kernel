...
static int __devinit wk2124B_probe(struct spi_device *spi)
{
#ifdef _DEBUG_WK2XXX
	printk(KERN_ALERT "-wk2124B_probe()------in---\n");
#endif
	uint8_t i;
    int status;
...
static int __devexit wk2124B_remove(struct spi_device *spi)
{
...
static int wk2124B_resume(struct spi_device *spi)
{
	printk(KERN_ALERT "resume wk2xxx");
	return 0;
}

static struct spi_driver wk2124B_driver = {
    .driver = {
        .name  = "wk2124B",
        .bus   = &spi_bus_type,
        .owner = THIS_MODULE,
    ...
    .probe  = wk2124B_probe,
    .remove = __devexit_p(wk2124B_remove),
    .resume = wk2124B_resume,
};

static int __init wk2124B_init(void)
{
    int retval;

    retval = spi_register_driver(&wk2124B_driver);
    printk(KERN_ALERT "rgister spi return v = :%d\n", retval);

	return retval;
}

static void __exit wk2124B_exit(void)
{
    spi_unregister_driver(&wk2124B_driver);
    printk("TEST_REG:quit ");
}

module_init(wk2124B_init);
module_exit(wk2124B_exit);
