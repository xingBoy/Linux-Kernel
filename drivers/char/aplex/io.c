...
#define CONFIG_MCASP0_FSX_offset            0x994
#define CONFIG_MCASP0_AXR0_offset           0x998
#define CONFIG_MCASP0_AHCLKR_offset         0x99c
...
static int aplex_io_set_init(void)
{
	...
	/*pull down*/
	__raw_writel(0x27 ,(base + CONFIG_GPMC_AD12_offset		 ));
	__raw_writel(0x27 ,(base + CONFIG_GPMC_AD13_offset		 ));
	__raw_writel(0x27 ,(base + CONFIG_GPMC_AD14_offset		 ));
	__raw_writel(0x27 ,(base + CONFIG_GPMC_AD15_offset		 ));
...
{
	if(register_chrdev_region(MKDEV(304,0),1,"io_ctl") < 0)
	{
		return 	-ENOMEM	;
	}
	aplex_io = cdev_alloc();
