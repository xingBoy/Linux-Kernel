...
#define UART3_RTS_CTL GPIO_TO_PIN(3, 2)
#elif defined(CONFIG_OK335XS)
#define UART3_RTS_CTL GPIO_TO_PIN(0, 31)
#endif
...
static ssize_t gpio_state_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
        ...
        gpio_direction_output(UART3_RTS_CTL, gpio_state);
...
static int __init io_ctl_dev_init(void)
{
	...
	#if defined(CONFIG_OK335XD)
	ret = gpio_request(UART3_RTS_CTL, "gpio3_2");
	#elif defined(CONFIG_OK335XS)
	ret = gpio_request(UART3_RTS_CTL, "gpio0_31");
	#endif
	if(ret < 0)
