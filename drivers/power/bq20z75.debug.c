...
{
    printk(KERN_ERR "%d, %s\n", __LINE__, __func__);
    printk(KERN_ERR "client->address: %d, address: %d\n", client->addr, address);
    ...
    for (count = 0; count < 127; count++)
    {
        ...
        */
	    ret = bq20z75_read_word_data(client,
		    bq20z75_data[REG_MANUFACTURER_DATA].addr);
	    if (ret < 0) {
            //printk(KERN_ERR "read error, count:%d\n", count);
		    return ret;
        }
        /*
        else {
            printk(KERN_ERR "read success, count:%d\n", count);
        }
        unsigned long delay = jiffies + INTERVEL;
...
{
    printk(KERN_ERR "%d, %s: baterry_init\n", __LINE__, __func__);
