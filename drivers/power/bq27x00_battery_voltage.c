...
static struct dm_reg bq274xx_dm_regs[] = {
    {82, 0, 2, 4200},   /* Qmax */
    {82, 5, 1, 0x81},   /* Load Select */
    {82, 10, 2, 4200},  /* Design Capacity */
    {82, 12, 2, 10000}, /* Design Energy */
    {82, 16, 2, 2700},  /* Terminate Voltage */
    {82, 27, 2, 100},   /* Taper rate */
};
...
static struct dm_reg bq276xx_dm_regs[] = {
	...
	{82, 0, 2, 1000},	/* Qmax */
	{82, 2, 1, 0x81},	/* Load Select */
	{82, 3, 2, 3800},	/* Design Capacity */
	{82, 5, 2, 14060},	/* Design Energy */
	{82, 9, 2, 3200},	/* Terminate Voltage */
	{82, 20, 2, 260},	/* Taper rate */
};
	...
	u8 offset, u8 *data)
{
	...
	print_buf(__func__, buf);
	if (memcmp(data, buf, 32)) {
		dev_err(di->dev, "%s: error updating subclass %d offset %d\n",
			__func__, subclass, offset);
		printk("zengjf check %s position compare :false.\n", __func__);
		return 0;
	} else {
		printk("zengjf check %s position compare :true.\n", __func__);
		return 1;
	}
}
...
static int bq27x00_battery_read_soc(struct bq27x00_device_info *di)
{
	...
	if ( volt < 3580 ) 
		soc = 0;
	else if (volt >= 3580 && volt < 3630) 
		soc = 2;
	else if (volt >= 3630 && volt < 3680)
		soc = 5;
	else if (volt >= 3680 && volt < 3740)
		soc = 10;
	else if (volt >= 3740 && volt < 3770)
		soc = 20;
	else if (volt >= 3770 && volt < 3790)
		soc = 30;
	else if (volt >= 3790 && volt < 3820)
		soc = 40;
	else if (volt >= 3820 && volt < 3870)
		soc = 50;
	else if (volt >= 3870 && volt < 3920)
		soc = 60;
	else if (volt >= 3920 && volt < 3980)
		soc = 70;
	else if (volt >= 3980 && volt < 4060)
		soc = 80;
	else if (volt >= 4060 && volt < 4200)
		soc = 90;
	else if (volt >= 4200)
		soc = 100;

	printk("------------------------------------------------\n");
	printk("zengjf check position soc: %d.\n", soc);
	printk("zengjf check position volt: %d.\n", volt);
	printk("------------------------------------------------\n");
...
static int bq27x00_battery_read_dcap(struct bq27x00_device_info *di)
{
	...
	printk("zengjf Design Capacity: %d in %s.\n", dcap, __func__);
...
static void bq27x00_update(struct bq27x00_device_info *di)
{
	...
	if (cache.flags >= 0) {
		...
		printk("---------------------------------------\n");
		printk("+++++++++++++++++++++++++++++++++++++++\n");
		printk("cache.NominalAvailableCapacity : %d\n", bq27xxx_read(di, 0x08, false));
		printk("cache.FullAvailableCapacity : %d\n", bq27xxx_read(di, 0x0A, false));
		printk("cache.RemainingCapacity : %d\n", bq27xxx_read(di, 0x0c, false));
		printk("cache.FullChargeCapacity : %d\n", bq27xxx_read(di, 0x0e, false));
		printk("OpConfig : %x\n", bq27xxx_read(di, 0x3a, false));
		printk("cache.charge_full : %d\n", bq27xxx_read(di, 0x0e, false));
		printk("cache.health : %d\n", bq27xxx_read(di, 0x20, false));
		printk("cache.tmperature : %d\n", bq27xxx_read(di, 0x02, false));
		printk("cache.power_avg : %d\n", bq27xxx_read(di, 0x04, false));
		printk("di->charge_design_full : %d\n", bq27xxx_read(di, 0x3d, false));
		printk("+++++++++++++++++++++++++++++++++++++++\n");
	}
...
static void rom_mode_gauge_dm_init(struct bq27x00_device_info *di)
{
	...
	for (i = 0; i < di->dm_regs_count; i++) {
		...
		printk("zengjf check bq27441 data: %d.\n", dm_reg->data);
		...
		if (blk_number == blk_number_prev) {
			...
			printk("zengjf check %s position 1.\n", __func__);
		} else {
			...
			copy_to_dm_buf_big_endian(di, buf, offset,
				dm_reg->len, dm_reg->data);
			printk("zengjf check %s position 2.\n", __func__);
		}
	...
	printk("zengjf check buf_valid = %s in %s.\n", buf_valid ? "true" : "false", __func__);
...
static void bq27x00_battery_poll(struct work_struct *work)
{
	...
	bq27x00_update(di);
	printk("zengjf check %s position.\n", __func__);
