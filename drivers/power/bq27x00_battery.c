...
//#define SABRESD_GAUGE_PIN        IMX_GPIO_NR(1, 29)
...
#if 0
static __initdata u8 bq27541_regs[NUM_REGS] = {
    0x00,   /* CONTROL -NA */
    0x02,   /* AR       */
    0x04,   /* ARTTE    */
    0x06,   /* TEMP     */
...
enum bq27x00_chip { BQ27200, BQ27500, BQ27520, BQ274XX, BQ276XX, BQ2753X, BQ27541};
...
static __initdata enum power_supply_property bq27541_battery_props[] = {
    POWER_SUPPLY_PROP_STATUS,
    POWER_SUPPLY_PROP_PRESENT,
    POWER_SUPPLY_PROP_VOLTAGE_NOW,
    POWER_SUPPLY_PROP_CURRENT_NOW,
...
static struct dm_reg bq274xx_dm_regs[] = {
    ...
    // {82, 10, 2, 4500},  /* Design Capacity */
    {82, 10, 2, 5500},  /* Design Capacity */
    // {82, 12, 2, 17100},  /* Design Energy */
    {82, 12, 2, 20900},  /* Design Energy */
    {82, 16, 2, 3580},  /* Terminate Voltage */
    {82, 27, 2, 1000},   /* Taper rate */
};
...
static struct dm_reg bq27541_dm_regs[] = {
    {82, 0, 2, 5500},   /* Qmax */
    {80, 0, 1, 0x81},   /* Load Select */
    {48, 23, 2, 4500},  /* Design Capacity */
    // {82, 12, 2, 17100},  /* Design Energy */
    //{82, 12, 2, 20900},  /* Design Energy */
    {80, 48, 2, 3580},  /* Terminate Voltage */
//    {82, 27, 2, 1000},   /* Taper rate */
};
...
static void bq27x00_update(struct bq27x00_device_info *di)
{
    ...
    /*
    printk("zengjf <%s> cache.temperature       = %d.\n", __func__, cache.temperature);
    printk("zengjf <%s> cache.time_to_empty     = %d.\n", __func__, cache.time_to_empty);
    printk("zengjf <%s> cache.time_to_empty_avg = %d.\n", __func__, cache.time_to_empty_avg);
    printk("zengjf <%s> cache.time_to_full      = %d.\n", __func__, cache.time_to_full);
    printk("zengjf <%s> cache.charge_full       = %d.\n", __func__, cache.charge_full);
    printk("zengjf <%s> cache.cycle_count       = %d.\n", __func__, cache.cycle_count);
    printk("zengjf <%s> cache.capacity          = %d.\n", __func__, cache.capacity);
    printk("zengjf <%s> cache.energy            = %d.\n", __func__, cache.energy);
    printk("zengjf <%s> cache.flags             = %d.\n", __func__, cache.flags);
    printk("zengjf <%s> cache.power_avg         = %d.\n", __func__, cache.power_avg);
    printk("zengjf <%s> cache.health            = %d.\n", __func__, cache.health);
    */
    /*
    ...
    */
    ...
    printk("zengjf <%s> bq27x00_battery_voltage = %d.\n", __func__, val.intval);
}
...
static int __init bq27x00_powersupply_init(struct bq27x00_device_info *di)
{
    ...
    if (di->chip == BQ274XX) {
        ...
        set_properties_array(di, bq27541_battery_props,
            ARRAY_SIZE(bq27541_battery_props));
    } else {
                 ...
                 const struct i2c_device_id *id)
{
    ...
    if (di->chip == BQ27200)
        ...
        regs = bq27541_regs;
        di->dm_regs = bq27541_dm_regs;
        di->dm_regs_count = ARRAY_SIZE(bq27541_dm_regs);
    } else {
        dev_err(&client->dev,
    ...
    // 2016-7-15 zengjf add for irq
    /*
    gpio_request(SABRESD_GAUGE_PIN, "SABRESD_GAUGE_PIN");
    gpio_direction_input(SABRESD_GAUGE_PIN);
    unsigned int irq = gpio_to_irq(SABRESD_GAUGE_PIN);
    int ret = request_irq(irq, bq27xxx_battery_irq_handler_thread,
            IRQF_TRIGGER_RISING,
             "SABRESD_GAUGE_PIN", di);
    enable_irq_wake(irq);
    */
