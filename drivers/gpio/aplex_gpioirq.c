...
int demo_probe(struct platform_device * pdevice)
{
    ...
    //request_irq(gpio_to_irq(gpio_nu),irq_handle, 0, "palmz71-cable", NULL))
    ret = request_irq(gpio_to_irq(gpio_nu), reset_ip_handler,out_value[1],"reset_ip", NULL);
    //ret = request_irq(gpio_to_irq(gpio_nu), reset_ip_handler, IRQF_TRIGGER_FALLING,"reset_ip", NULL);
    if(ret)
    {
