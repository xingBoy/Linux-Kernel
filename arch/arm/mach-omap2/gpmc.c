...
#else
	//	gpmc_cs_write_reg(cs, GPMC_CS_CONFIG7, 0);
	//	gpmc_cs_write_reg(cs, GPMC_CS_CONFIG1, 0x00000800);
		gpmc_cs_write_reg(0, GPMC_CS_CONFIG2, 0x001e1e00);
		gpmc_cs_write_reg(0, GPMC_CS_CONFIG3, 0x001e1e00);
		gpmc_cs_write_reg(0, GPMC_CS_CONFIG4, 0x16051807);
		gpmc_cs_write_reg(0, GPMC_CS_CONFIG5, 0x00151e1e);
		//gpmc_cs_write_reg(cs, GPMC_CS_CONFIG6, 0x16000f80);
		base = 0xD8000000;
		size = 0;
		//0x00000008
//		gpmc_cs_write_reg(cs, GPMC_CS_CONFIG7, (((size & 0xF) << 8) | ((base >> 24) & 0x3F) |
//				                             (1 << 6)));
#endif
	if (cpu_is_omap34xx()) {
	...
	//	printk(KERN_INFO "Danny zhao test -- GPMC CS%d CLK period is %lu ns (div %d)\n",
	//			cs, (div * gpmc_get_fclk_period()) / 1000, div);
