...
static struct omap_opp_def __initdata am33xx_es2_1_opp_list[] = {
	...
	/* MPU OPP4 - OPPNitro */
	OPP_INITIALIZER("mpu", true, 1000000000,
...
int __init omap3_opp_init(void)
{
	...
	if (cpu_is_omap3630())
		...
		rev = omap_rev();
		pr_warn("<danny debug> opp_init, cpu rev = %x \n",rev);
		switch (rev) {
		...
		case AM335X_REV_ES2_1:
			...
			pr_warn("<danny debug> opp_init, cpu max_freq = %x \n",max_freq);
			...
			pr_warn("<danny debug> opp_init, cpu 1G freq = %d \n", am33xx_es2_1_opp_list[OPP_NITRO_1GHZ_INDEX].default_available);
