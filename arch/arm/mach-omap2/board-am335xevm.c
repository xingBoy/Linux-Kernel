...
#define SCREEN_SIZE_1024x768_12_1 9
...
struct da8xx_lcdc_platform_data  NHD_1024768MF_ATXI_pdata_12_1 = {
	.manu_name              = "NHD",
	.controller_data        = &lcd_cfg,
	.type                   = "APLEX_12_1_1024_768",
};
...
static struct pinmux_config mmc1_rst_pin_mux[] = {
...
static struct pinmux_config mmc1_cd_only_pin_mux[] = {
	{"gpmc_advn_ale.gpio2_2", OMAP_MUX_MODE7 | AM33XX_PIN_OUTPUT },
	{NULL, 0},
};
...
/*pwm for buzzer*/
/**
 * zengjf 2015-8-25 modify for buzzer
 */
...
//	screen_size = SCREEN_SIZE_1024x768_12_1;
	...
	else if(screen_size == SCREEN_SIZE_1024x768_12_1 )
		lcdc_pdata = &NHD_1024768MF_ATXI_pdata_12_1 ;
    else
...
static struct pinmux_config mii1_non_pin_mux[] = {
	{"mii1_col.mii1_col", OMAP_MUX_MODE0 | AM33XX_PIN_OUTPUT},
    {NULL, 0},
};
static void mii1_init(int evm_id, int profile)
{
//	setup_pin_mux(mii1_non_pin_mux);
...
static void mmc1_init(int evm_id, int profile)
{
	...
	case BEAGLE_BONE_BLACK:
		...
		setup_pin_mux(mmc1_rst_pin_mux);
		gpio_request(GPIO_TO_PIN(0,27), "gpio0-17");
...
static void gpio_keys_init_forlinx(int evm_id, int profile)
{
	int err;
...
/*uart2  com2*/
static void uart2_init_s(int evm_id, int profile)
{
...
static void gpio_keys_init_forlinx_s(int evm_id, int profile)
{
        int err;

        setup_pin_mux(gpio_keys_pin_mux_forlinx_s);
        err = platform_device_register(&am335x_evm_gpio_keys_forlinx_s);
        if (err)
                pr_err("failed to register gpio key device\n");
}
/*keys button*/
...
static struct gpio_led gpio_leds_s[] = {
        {
                .name                   = "usr0",
...
static struct gpio_led_platform_data gpio_led_info_s = {
        .leds           = gpio_leds_s,
        .num_leds       = ARRAY_SIZE(gpio_leds_s),
};

static struct platform_device leds_gpio_s = {
        .name   = "leds-gpio",
        .id     = -1,
        .dev    = {
                .platform_data  = &gpio_led_info_s,
        },
};

static void gpio_led_init_s(int evm_id, int profile)
{
        int err;

        setup_pin_mux(gpio_led_mux_s);
        err = platform_device_register(&leds_gpio_s);
        if (err)
                pr_err("failed to register gpio led device\n");
}
/*configure   for   forlinx   OK335xS     end */
...
static struct evm_dev_cfg sbc_7109_dev_cfg[] = {
	...
	//{uart2_init,    DEV_ON_BASEBOARD, PROFILE_ALL},//fixed
	...
	{uart2_init,    DEV_ON_BASEBOARD, PROFILE_ALL},//fixed
	{uart4_init,    DEV_ON_BASEBOARD, PROFILE_ALL},//fixed
	{spi1_init,     DEV_ON_BASEBOARD, PROFILE_ALL},//fixed
...
static void setup_sbc_7109(void)
{
	pr_info("The board is a sbc-7109.\n");
	...
	_configure_device(EVM_SK, sbc_7109_dev_cfg, PROFILE_NONE);
