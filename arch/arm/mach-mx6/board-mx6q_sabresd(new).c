...
#endif
/**
 * 2015-8-4 zengjf add for wake on lan
 * referrence: https://community.freescale.com/thread/315218
 */
//#define MX6_WOL_IRQ         IMX_GPIO_NR(1, 24)		
...
{
    unsigned short val;
	unsigned char *addr = phydev->attached_dev->dev_addr;
	...
	/*
 	phy_write(phydev, 0xd, 0x3);
 	phy_write(phydev, 0xe, 0x804A);
 	phy_write(phydev, 0xd, 0xc003);
...
#endif
		/**
		 * zengjf 2015-8-5 add for wake on lan
		 */
		//.wol_irq = MX6_WOL_IRQ
};
...
/*	if (uart5_enabled) {
	...
	/**
	 * zengjf 2015-7-23 commented v4l2
	 */
