...
//#define SABRESD_SCREENLOCK	IMX_GPIO_NR(3, 29)
...
// Tony 2016-12-26
//#define LCD_RESET               IMX_GPIO_NR(4, 20)
/* matrix keypad */
// COL
...
/* wifi */
#define SABRESD_WIFI_CHIP_EN    IMX_GPIO_NR(2, 2)
#define SABRESD_BT_HOST_WAKE    IMX_GPIO_NR(2, 3)
#define SABRESD_UART3_WAKE      IMX_GPIO_NR(2, 4)
#define SABRESD_SDIO_WAKE       IMX_GPIO_NR(2, 5)
#define SABRESD_SDIO_RESET      IMX_GPIO_NR(2, 6)
#define SABRESD_W_DISABL2       IMX_GPIO_NR(2, 7)

/* AD7606 */
#define AD7606_GPIO_CONVST      IMX_GPIO_NR(1, 31)
#define AD7606_GPIO_RESET       IMX_GPIO_NR(6, 10)
#define AD7606_GPIO_OS0         IMX_GPIO_NR(6, 8)
#define AD7606_GPIO_OS1         IMX_GPIO_NR(6, 7)
#define AD7606_GPIO_OS2         IMX_GPIO_NR(6, 9)
#define AD7606_GPIO_FRSTDATA    IMX_GPIO_NR(1, 23)
#define AD7606_GPIO_STBY        IMX_GPIO_NR(1, 22)
#define AD7606_GPIO_BUSY        IMX_GPIO_NR(1, 25)

#define SENSOR_PWR_EN           IMX_GPIO_NR(3, 19)
...
static struct gpio_keys_button new_sabresd_buttons[] = {
	...
	//GPIO_BUTTON(SABRESD_SCREENLOCK, KEY_SCREENLOCK, 1, "screenlock-key", 1, 1),
...
/*
    ret = gpio_request(LCD_RESET, "lcd reset");
    if (ret) {
        printk(KERN_ERR"request LCD RESET error!!\n");
        return;
    } else {
...
/*	ret = gpio_request(SABRESD_PFUZE_INT, "pFUZE-int");
    ...
    gpio_request(SENSOR_PWR_EN, "sensor pwr en"); 
...
/*	gpio_request(SABRESD_AUX_5V_EN, "aux_5v_en");
	...
	gpio_request(SABRESD_GPS_RESET, "gps_reset");
	gpio_direction_output(SABRESD_GPS_RESET, 1);
	gpio_free(SABRESD_GPS_RESET);
...
/*		gpio_request(SABRESD_ELAN_INT, "elan-interrupt");
    ...
    */
	...
	ret = gpio_request(SABRESD_WIFI_CHIP_EN, "SABRESD wifi chip en");
	if ( ret ) {
		printk("get SABRESD_WL_RESET gpio FAILED!\n");
		return ret;
	}
    gpio_direction_output(SABRESD_WIFI_CHIP_EN, 1);
	gpio_free(SABRESD_WIFI_CHIP_EN);

	ret = gpio_request(SABRESD_BT_HOST_WAKE, "SABRESD bluetooth host wake");
	if ( ret ) {
		printk("get SABRESD_WL_RESET gpio FAILED!\n");
		return ret;
	}
    gpio_direction_output(SABRESD_BT_HOST_WAKE, 1);
	gpio_free(SABRESD_BT_HOST_WAKE);

	ret = gpio_request(SABRESD_UART3_WAKE, "SABRESD wifi chip en");
	if ( ret ) {
		printk("get SABRESD_WL_RESET gpio FAILED!\n");
		return ret;
	}
    gpio_direction_output(SABRESD_UART3_WAKE, 0);       // active low
	gpio_free(SABRESD_UART3_WAKE);

	ret = gpio_request(SABRESD_SDIO_WAKE, "SABRESD wifi chip en");
	if ( ret ) {
		printk("get SABRESD_WL_RESET gpio FAILED!\n");
		return ret;
	}
    gpio_direction_output(SABRESD_SDIO_WAKE, 1);    // active high
	gpio_free(SABRESD_SDIO_WAKE);
		

	ret = gpio_request(SABRESD_SDIO_RESET, "SABRESD wifi chip en");
	if ( ret ) {
		printk("get SABRESD_WL_RESET gpio FAILED!\n");
		return ret;
	}
    gpio_direction_output(SABRESD_SDIO_RESET, 1);   // active low
	gpio_free(SABRESD_SDIO_RESET);

	ret = gpio_request(SABRESD_W_DISABL2, "SABRESD wifi chip en");
	if ( ret ) {
		printk("get SABRESD_WL_RESET gpio FAILED!\n");
		return ret;
	}
    gpio_direction_output(SABRESD_W_DISABL2, 1);    // active low, high to enable BT
	gpio_free(SABRESD_W_DISABL2);
